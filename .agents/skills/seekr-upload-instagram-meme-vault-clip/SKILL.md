---
name: seekr-upload-instagram-meme-vault-clip
description: Upload a meme vault clip to Instagram as a Reel.
---
...
1. **Find available env files** using Glob:
   ...
   ```
   Glob pattern: modules/meme-vault/.env*
   ```
...
```bash
cd modules/meme-vault && npx tsx independent_node_skills/upload-instagram.ts \

---
name: delete-meme
description: Delete a meme from Meme Vault (Supabase, GitHub, Instagram)
model: haiku
---
...
```bash
cd modules/meme-vault && npx tsx -e "
...
```bash
cd modules/meme-vault && npx tsx independent_node_skills/delete-meme.ts \
  --clip-id "<CLIP_ID>" \
  --env-path .env
```
